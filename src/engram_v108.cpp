...
int main() {
  ...
  for(size_t i=0;i<100000;++i){ uint8_t truth=(uint8_t)((i/5+1)%32); auto q=bank[truth]; for(auto& v:q)v=(int8_t)(v+noise(rng)); int raw=0,best=1<<30; for(int s=0;s<32;++s){int dist=0;for(int d=0;d<16;++d)dist+=std::abs((int)q[d]-bank[s][d]);if(dist<best){best=dist;raw=s;}} int aided=raw; uint8_t successor=0,confidence=0; if(memory.lookup(q,successor,confidence)&&confidence>=2&&successor<32){aided=successor;++used;} if(raw==(int)truth)++raw_correct; if(aided==(int)truth)++aided_correct; if(aided!=raw&&aided==(int)truth&&raw!=(int)truth){} if(aided!=raw&&aided!=(int)truth&&raw==(int)truth)++harmed; memory.update(q,truth); previous=aided;has_previous=true; (void)previous;(void)has_previous; }
